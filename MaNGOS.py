...
def clean_MaNGOS():
    ...
    os.system('cd '+new_install_dir+'/etc/;cp mangosd.conf.dist mangosd.conf;cp realmd.conf.dist realmd.conf;cp scriptdev2.conf.dist scriptdev2.conf')
    print ""
    ...
    if database=='default':
       ...
       for line in os.popen('cd '+new_install_dir+'/database/;git clone '+git_database):
           if os.path.exists(new_install_dir+'/database/database'):
              ...
              database_install(db_host,db_user,db_pass)
              exit()
    elif database=='ytdb':
...
def database_install():
    ...
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' < '+new_work_dir+'/sql/create_mysql.sql')
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' characters < '+new_work_dir+'/sql/characters.sql')
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' mangos < '+new_work_dir+'/sql/mangos.sql')
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' realmd < '+new_work_dir+'/sql/realmd.sql')
    #ScriptDev2  Setup
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' < '+new_work_dir+'/src/bindings/ScriptDev2/sql/scriptdev2_create_database.sql')
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' scriptdev2 < '+new_work_dir+'/src/bindings/ScriptDev2/sql/scriptdev2_create_structure_mysql.sql')
    os.system('mysql -h '+db_host+' -u '+db_user+' -p'+db_pass+' scriptdev2 < '+new_work_dir+'/src/bindings/ScriptDev2/sql/scriptdev2_script_full.sql')
