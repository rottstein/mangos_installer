...
def Classic(self):
    ...
    if self.checkFolder(self.backup_dir)==1:
       ...
       self.mkdir(self.backup_dir)
    self.msg('\nDownloading MaNGOS!','green')
    self.fetch_git(self,classic[0])
    if self.checkFolder(self.work_dir+'/mangos-classic')==1:
       self.msg('MaNGOS Succesfully Downloaded!','green')
    else:
       self.msg('Error: Please check your connection and run script again!','red')  
       exit()
    if self.q_sd2=='yes':
       self.msg('\nDownloading ScriptDev2!','green')    
       self.fetch_scriptdev2(self,classic[1],'classic')
       if self.checkFolder(self.work_dir+'/mangos-classic/src/bindings/ScriptDev2')==1:
          self.msg('ScriptDev2 Succesfully Downloaded!','green')
       else:
          self.msg('Error: Please check your connection and run script again!','red')  
          exit()
    ...
    os.system('clear')
    self.msg('\nDownloading Databases.','green')
    self.fetch_database(self,classic[3],'classic')
    self.MaNGOS_Database(self)
    self.setupChar(self)
    self.setupClassicScriptDev2(self)
    if self.q_currentrealm=='yes':
       self.updateRealm(self)
    else:
       self.msg('\nCreating new realm db.','green')
       self.setupRealm(self)
    self.msg('\nSetting up ACID.','green')
    self.fetch_custom_git(self,self.install_dir+'/database/',classic[2])
    os.system('cd '+self.install_dir+'/database/acid/;git checkout classic;mysql -h '+self.q_host+' -u '+self.q_user+' -p'+self.q_pass+' '+self.q_world+' < acid_classic.sql')
    os.system('mysql -h '+self.q_host+' -u '+self.q_user+' -p'+self.q_pass+' '+self.q_world+' < '+self.work_dir+'/mangos-classic/src/bindings/ScriptDev2/sql/mangoszero_scriptname_full.sql')
    self.msg('\nSetting Config files.','green')
    self.msg('\nSetting DataDir = "." to DataDir = '+str(self.q_data_dir),'green')
    self.replaceAll(self.install_dir+'/etc/mangosd.conf','"."','"'+str(self.q_data_dir)+'"')
    self.msg('\nSetting WorldServerPort = 8085 to WorldServerPort = '+str(self.q_realmport),'green')
    ...
    self.msg('\nSetting CharactersDatabaseInfo = characters to CharacterDatabaseInfo = '+str(self.q_char),'green') 
    self.replaceAll(self.install_dir+'/etc/mangosd.conf','127.0.0.1;3306;mangos;mangos;characters','127.0.0.1;3306;mangos;mangos;'+str(self.q_char)+'')
    self.msg('\nSetting LoginDatabaseInfo = scriptdevzero to LoginDatabaseInfo = '+str(self.q_script)+' (scriptdevzero.conf)','green') 
    self.replaceAll(self.install_dir+'/etc/scriptdevzero.conf','127.0.0.1;3306;mangos;mangos;scriptdevzero','127.0.0.1;3306;mangos;mangos;'+str(self.q_script)+'')
    os.system('mv '+self.work_dir+'/mangos-classic/sql '+self.install_dir)
    if self.q_acc=='yes':
       self.msg('\nCreating new Account..','green')
       self.createAccount()
    else:
